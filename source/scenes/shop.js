...
export class Shop extends Phaser.Scene{
    ...
    preload(){
        ...
        for(let i = 0; i < buttons.length; i++) {
            this.load.image('shop_' + buttons[i], 'source/assets/shopInterface' + buttons[i] + '.png');
        }
    ...
    create(){
        ...
        this.weaponsButton = this.add.text(485, 90, 'XXXX', {fontSize: '40px'}).setDepth(-1).setInteractive();
        this.weaponsButton.on('pointerup', ()=>{
            this.loadShop('WEAPONS');
        })
        this.toolsButton = this.add.text(485, 170, 'XXXX', {fontSize: '40px'}).setDepth(-1).setInteractive();
        this.toolsButton.on('pointerup', ()=>{
            this.loadShop('TOOLS');
        })
        this.consumablesButton = this.add.text(485, 250, 'XXXX', {fontSize: '40px'}).setDepth(-1).setInteractive();
    ...
    loadShop(type) {
        ...
        this.add.image(0, 0, 'shop_'+type).setOrigin(0,0).setDepth(0);
